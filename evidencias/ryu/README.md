...
| 13 | 22/09 | [Baselines triviais e congelamento](2026-09-22-13-baselines-triviais-e-congelamento.md) | Ferramenta de congelamento por hash pronta (`prova_freeze.py`, checagem automática no runner). Baselines triviais medidos: palavra de alarme (0,571) e comprimento (0,592) bem abaixo do critério; **saco de palavras (Naive Bayes) em 0,912 — acima de 0,90**, mesmo depois de reescrever 20 textos que vazavam "agora"/"comendo". Sinal residual é estilo de autor único, espalhado por conectivos — recomendação real é diversificar quem escreve, não editar mais palavras. Congelamento não executado, decisão registrada como pendente do time |
| 14 | 22/09 | [Comparando Gemini e Ollama](2026-09-22-14-comparando-gemini-e-ollama.md) | Experimento isolado (nada em produção usa), 9 casos: Precision@1/MRR empatados tecnicamente (amostra pequena demais para decidir). Achado qualitativo forte: o Ollama alucinou termos inexistentes duas vezes em dois casos revisados ("doença de Huntington" numa convulsão canina, "hipotirese" numa intoxicação por cebola) — o Gemini não alucinou nenhuma vez e usou terminologia clinicamente correta e específica (corpúsculos de Heinz). Suíte do backend em 235 |
| 15 | 23/09 | [Gemini x Ollama, amostra ampliada](2026-09-23-15-gemini-x-ollama-amostra-ampliada.md) | Ampliado para 25 casos. Quantitativo misto: Gemini ganha no agregado (MRR 0,433 vs 0,365) mas Ollama vence mais casos individuais (11 vs 9) — sem vencedor limpo na recuperação, o gargalo é ordenação/reranking, não a consulta. Qualitativo confirma e piora: mais 3 alucinações do Ollama (sigla ortopédica inventada, contradição miose/midríase na mesma resposta, "Síndrome de Toxina de Gato" inexistente) — total 5 alucinações em 25 casos, zero no Gemini |
| 16 | 23/09 | [Gemini em produção, com fallback](2026-09-23-16-integracao-gemini-com-fallback.md) | **Integrado de verdade**: `HybridQueryClient` (Gemini com fallback para Ollama, exceto no HyDE) virou o cliente padrão do `ChatPipeline`; `HYDE_ENABLED` voltou a `True`. Confirmado ao vivo: etapa de consulta inteira em 3,3s (era dezenas de segundos no Ollama). No caminho, uma imagem Docker desatualizada derrubou o backend — corrigido com rebuild completo, registrado como B-58. Suíte do backend em 242 |
